	...
	switch ($_GET['view']) {
		case '':
			...
			if($system['forums_statistics_enabled']) {
				/* total threads */
				$get_threads = $db->query("SELECT * FROM forums_threads") or _error(SQL_ERROR_THROWEN);
		    	$insights['threads'] = $get_threads->num_rows;
		    	/* total replies */
				$get_replies = $db->query("SELECT * FROM forums_replies") or _error(SQL_ERROR_THROWEN);
		    	$insights['replies'] = $get_replies->num_rows;
				/* total users */
				$get_users = $db->query("SELECT * FROM users") or _error(SQL_ERROR_THROWEN);
