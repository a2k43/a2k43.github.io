...
try {
	...
	$get_group = $db->query(sprintf("SELECT groups.*, posts_photos.source as group_picture_full, groups_categories.category_name as group_category_name FROM groups LEFT JOIN posts_photos ON groups.group_picture_id = posts_photos.photo_id LEFT JOIN groups_categories ON groups.group_category = groups_categories.category_id WHERE groups.group_name = %s", secure($_GET['username']))) or _error(SQL_ERROR_THROWEN);
