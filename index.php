...
try {
	...
	if(!$user->_logged_in) {
		...
		if($system['system_random_profiles']) {
			...
			$where = ($system['activation_enabled'])? " AND user_activated = '1'" : " ";
			$get_random_profiles = $db->query("SELECT user_name, user_firstname, user_lastname, user_gender, user_picture FROM users WHERE user_banned = '0'".$where."ORDER BY RAND() LIMIT 4") or _error(SQL_ERROR_THROWEN);
			if($get_random_profiles->num_rows > 0) {
				while($random_profile = $get_random_profiles->fetch_assoc()) {
					$random_profile['user_picture'] = get_picture($random_profile['user_picture'], $random_profile['user_gender']);
