...
try {
	...
	$get_page = $db->query(sprintf("SELECT pages.*, posts_photos.source as page_picture_full, pages_categories.category_name as page_category_name FROM pages LEFT JOIN posts_photos ON pages.page_picture_id = posts_photos.photo_id LEFT JOIN pages_categories ON pages.page_category = pages_categories.category_id WHERE pages.page_name = %s", secure($_GET['username']))) or _error(SQL_ERROR_THROWEN);
...
// page header
page_header($spage['page_title'], $spage['page_description']);
